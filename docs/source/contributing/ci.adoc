...
It is the only CI system which validates the xref:../contributing/developing.adoc#commit-message-requirements[Commit Message Requirements] on each pull request.
|link:https://circleci.com/gh/minishift/minishift/tree/master[master and pull requests]
|link:https://github.com/minishift/minishift/blob/master/.circleci/config.yml[config.yml]
